...
class ModelInstanceEndpoint():
    ...
    def save_ptmodel(self, modelPath):
        """
        Creates and returns a Task which saves the model to the path provided
        ...
        :return: The created Task object
        """
        logger.debug("Save PyTorch model called")
        try:
            task = ModelInstanceTask(operation="save_ptmodel",
                                     parameters={"path": modelPath})
            self.add_task(task)
        except RuntimeError as inst:
            msg = "Error while saving pytorch model to path provided: %s" % str(inst)
            logger.exception(msg)
            return ServiceError(500, msg), 500
    ...
    def add_task(self, task):
        ...
        try:
            ...
            ops = ['initialize', 'load_data', 'build_model', 'train', 'pause', 
                   ...
                   'generate', 'save_generations', "miniaturize"]
